...
// Pages site (https://savagemanage.github.io/open-games/gridfall/), so a
// production build must resolve assets under '/open-games/gridfall/'. Local dev
// serves from the root, hence the '/' branch.
export default defineConfig(({ command }) => ({
  base: command === 'build' ? '/open-games/gridfall/' : '/',
  build: {
    outDir: 'dist',
    assetsDir: 'assets',
    sourcemap: false,

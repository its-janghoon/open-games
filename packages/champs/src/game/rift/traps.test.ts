...
function candidate(id: string, x: number, y = 500, damageable = true): TrapCandidate {
  return { id, pos: { x, y }, damageable };
}

describe('resolveTraps', () => {
  it('catches someone standing inside the radius and consumes the trap', () => {
    const result = resolveTraps([trap()], [candidate('e1', 520)], 5);
  ...
  it('survives untriggered when nobody is inside', () => {
    const result = resolveTraps([trap()], [candidate('e1', 900)], 5);
  ...
  it('drops an expired trap WITHOUT firing it, even with someone standing on it', () => {
     ...
     */
    const result = resolveTraps([trap({ expiresAt: 4 })], [candidate('e1', 500)], 5);
    expect(result.triggers).toHaveLength(0);
    expect(result.traps).toHaveLength(0);
  });

  it('treats the expiry instant itself as expired', () => {
    expect(resolveTraps([trap({ expiresAt: 5 })], [candidate('e1', 500)], 5).triggers).toHaveLength(0);
    expect(resolveTraps([trap({ expiresAt: 5.001 })], [candidate('e1', 500)], 5).triggers).toHaveLength(1);
  });

  it('ignores anyone the caller says is not damageable', () => {
    const result = resolveTraps([trap()], [candidate('friend', 500, 500, false)], 5);
    expect(result.triggers).toHaveLength(0);
  ...
  it('catches the nearest, breaking ties by id rather than array order', () => {
    ...
    expect(resolveTraps([trap()], [far, near], 5).triggers[0].targetId).toBe('zz');

    const tieA = candidate('aaa', 550);
    const tieB = candidate('bbb', 450);
    const forward = resolveTraps([trap()], [tieB, tieA], 5).triggers[0].targetId;
    const reverse = resolveTraps([trap()], [tieA, tieB], 5).triggers[0].targetId;
  ...
  it('processes traps in id order regardless of array order', () => {
    ...
    const forward = resolveTraps([b, a], [candidate('e1', 500)], 5);
    const reverse = resolveTraps([a, b], [candidate('e1', 500)], 5);
  ...
  it('respects the radius boundary', () => {
    expect(resolveTraps([trap()], [candidate('e1', 620)], 5).triggers).toHaveLength(1);
    expect(resolveTraps([trap()], [candidate('e1', 621)], 5).triggers).toHaveLength(0);
  });

  it('carries the slow through to the trigger, so the caller need not re-read the trap', () => {
    const result = resolveTraps([trap({ slowPercent: 0.5, slowDuration: 2 })], [candidate('e1', 500)], 5);
  ...
  it('does not mutate its inputs and is deterministic', () => {
    ...
    const a = resolveTraps(traps, candidates, 5);
    const b = resolveTraps(traps, candidates, 5);
  ...
  it('returns a trap whose point cannot be mutated back into the original', () => {
    const traps = [trap()];
    const result = resolveTraps(traps, [candidate('e1', 900)], 5);
